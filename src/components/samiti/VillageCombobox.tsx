...
import { filterVillageOptions, VILLAGE_OPTIONS } from '@/lib/villageOptions';
...
/**
 * गाँव picker: a dropdown of known villages that also accepts free
 * text, so custom village names and existing records keep working.
 */
export const VillageCombobox: React.FC<VillageComboboxProps> = ({
  value,
  onChange,
  className = '',
  ...
  const options = useMemo(
    () => (isOpen ? filterVillageOptions(value) : VILLAGE_OPTIONS),
    [isOpen, value]
  );
  ...
  const isKnown = VILLAGE_OPTIONS.some(o => o === value);
