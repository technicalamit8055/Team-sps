-- ==============================================================================
-- APPLY_EXPENSE_WRITE_ACCESS.sql
--
-- Fixes: a member with "Expense Manager" ticked in the access matrix cannot
-- record an expense, while an admin's expense shows up in that member's
-- account just fine.
--
-- Cause: samiti_expenses has no INSERT/UPDATE policy for workers -- only
-- SELECT -- so the read worked and every write was rejected by RLS. The
-- rejection was logged with console.warn and never shown, so the app reported
-- success and the row disappeared on reload.
--
-- Dashboard -> SQL Editor -> paste this whole file -> Run, with NOTHING
-- selected (the editor runs only the selection when there is one).
--
-- Idempotent: safe to re-run.
--
-- Source of truth: supabase/migrations/20260914140000_worker_expense_write_access.sql
-- ==============================================================================
...
-- ==============================================================================
-- Verify. Run this on its own afterwards, in a new tab.
--
-- Expect four rows: the original admin/manager ALL policy, the worker SELECT
-- policy, and the two new worker INSERT/UPDATE policies.
-- ==============================================================================
SELECT policyname, cmd
FROM pg_policies
WHERE schemaname = 'public' AND tablename = 'samiti_expenses'
ORDER BY policyname;
